...
export default function PainelMensagens() {
  ...
  const mensagens = useMensagens(categoria);
  ...
  function gerarPdf(mensagem, ocultar) {
    const doc = new jsPDF();
    let y = 20;
    ...
    doc.text(`Data: ${new Date(mensagem.dataCriacao?.toDate?.() || mensagem.dataCriacao).toLocaleDateString('pt-BR')}`, 130, y);
    ...
    if (mensagem.historico && mensagem.historico.length > 0) {
      mensagem.historico.forEach(h => {
        const linha = `[${new Date(h.data?.toDate?.() || h.data).toLocaleDateString('pt-BR')}] ${h.acao} por ${ocultar ? '***' : (h.usuario?.nome || 'Usuário')}${h.observacao ? ' - ' + h.observacao : ''}`;
  ...
  return (
    <div className="container mx-auto py-8">
      <TabsMensagens value={categoria} onChange={setCategoria} counts={counts} />
      <TabelaMensagens
        mensagens={mensagens}
        onVisualizar={setMensagemSelecionada}
        onArquivar={handleArquivar}
        onMigrar={handleMigrarPrompt}
        ...
        <div className="fixed inset-0 z-[99999] flex items-center justify-center">
          ...
          <div className="relative z-10 bg-white rounded-lg shadow-2xl max-w-3xl w-full print-modal flex flex-col max-h-[95vh] pointer-events-auto border-2 border-taxi-green">
            ...
            <div className="flex-1 overflow-y-auto p-4">
              <div id="print-area">
                <ImpressaoMensagem mensagem={imprimir.mensagem} ocultarDados={true} className="print:p-0 print:max-w-full print:mx-0 print:break-inside-avoid" />
              </div>
            </div>
